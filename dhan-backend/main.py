...
DIST_DIR = BASE_DIR.parent / "dhan-frontend" / "dist"

# 2a) Mount the assets generated by Vite
if DIST_DIR.exists():
    app.mount("/assets", StaticFiles(directory=DIST_DIR / "assets"), name="assets")

    # 2b) Serve the SPA index.html
    @app.get("/", include_in_schema=False)
    def index():
        return FileResponse(DIST_DIR / "index.html")

    # 2c) SPA fallback for client-side routes like /orders, /positions
    @app.get("/{full_path:path}", include_in_schema=False)
    def spa_fallback(full_path: str):
        file_path = DIST_DIR / full_path
        if file_path.is_file():
            # Serve real files if they exist (e.g., /robots.txt, /favicon.svg)
